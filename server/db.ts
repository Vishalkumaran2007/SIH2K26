...
import { EmailArtifact, InsertUser, Investigation, emailArtifacts, indicators, investigationEvents, investigationNotes, investigations, ipGeolocations, ipReputations, users } from "../drizzle/schema";
import { ENV } from "./_core/env";
import type { ParsedEmailAnalysis } from "./emailAnalysis";
...
export async function getInvestigation(userId: number, id: number) { const db = await requireDb(); const investigation = (await db.select().from(investigations).where(eq(investigations.id, id)).limit(1))[0]; if (!investigation || investigation.userId !== userId) return null; const artifact = (await db.select().from(emailArtifacts).where(eq(emailArtifacts.investigationId, id)).limit(1))[0]; const iocs = await db.select().from(indicators).where(eq(indicators.investigationId, id)); const events = await db.select().from(investigationEvents).where(eq(investigationEvents.investigationId, id)).orderBy(desc(investigationEvents.createdAt)); const notes = await db.select().from(investigationNotes).where(eq(investigationNotes.investigationId, id)).orderBy(desc(investigationNotes.createdAt)); const reputations = await db.select().from(ipReputations).where(eq(ipReputations.investigationId, id)).orderBy(desc(ipReputations.enrichedAt)); return { investigation, artifact, iocs, events, notes, reputations }; }
